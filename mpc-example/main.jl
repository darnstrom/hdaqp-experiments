...
## Setup MPC 
function mpc_experiments(;Np=30,Nc=Np,save_problems=true,plot=false) 
    Ts = 0.01;
    mpc = LinearMPC.MPC(Ac,Bc,Ts;C,Np,Nc);
    ...
    LinearMPC.add_constraint!(mpc, Au = [1.0;;], Aup=[-1.0;;], lb = [-0.1],ub = [0.1],prio=6)
    ...
    for prio in prios
        ...
        name = "scenario"*string(prio[1])*string(prio[2])*string(prio[3])
