...
int main() {
    ...
    output_file << "ratio "; 
    output_file << "daqpmin " << "lexlsmin " << "nipmmin " << "naivedaqpmin " << "tylermin ";
    output_file << "daqpmean " << "lexlsmean " << "nipmmean " << "naivedaqpmean " << "tylermean ";
    output_file << "daqpmax " << "lexlsmax " << "nipmmax " << "naivedaqpmax " << "tylermax";
    ...
    for (int i = 0; i < eq_probs.size(); i++){
        ...
        for (int run = 0; run < Nruns; run++){
            ...
            Eigen::VectorXi const normalized = normalized_breaks(h.breaks, h.matrix.rows());

            // DAQP
            DAQP daqp(n, h.upper.size(), mmax);
            auto t_start = std::chrono::high_resolution_clock::now();
            daqp.solve(h.matrix, h.upper, h.lower, normalized);
            ...
            // Tyler-HiGHS
            t_start = std::chrono::high_resolution_clock::now();
            TylerResult tyler = tyler_from_stack(h.matrix, h.upper, h.lower, h.breaks);
            t_end = std::chrono::high_resolution_clock::now();
            auto tyler_time = std::chrono::duration_cast<std::chrono::microseconds>(t_end - t_start);
            times(run,4) = std::chrono::duration<double>(tyler_time).count();

            // Compute slacks
            Eigen::VectorXd w_daqp = compute_band_slacks(h.matrix,h.upper,h.lower,daqp.get_primal());
            Eigen::VectorXd w_lexls = compute_band_slacks(h.matrix,h.upper,h.lower,lexls.get_x());
            Eigen::VectorXd w_nipm = compute_band_slacks(h.matrix,h.upper,h.lower,nipm.get_x());
            Eigen::VectorXd w_tyler = compute_band_slacks(h.matrix,h.upper,h.lower,tyler.primal);

            daqp_diff_lexls(run,i) = compute_lexdiff(w_daqp,w_lexls,h.breaks);
            daqp_diff_nipm(run,i) = compute_lexdiff(w_daqp,w_nipm,h.breaks);
            daqp_diff_tyler(run,i) = compute_lexdiff(w_daqp,w_tyler,h.breaks);
        }
    ...
    std::ofstream output_file_diff_tyler("daqp_diff_tyler.dat");
    output_file_diff_tyler << daqp_diff_tyler;
    output_file_diff_tyler.close();
}
